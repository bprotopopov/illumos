...
fzap_upgrade(zap_t *zap, dmu_tx_t *tx, zap_flags_t flags)
{
	 ...
	 */
	bzero(ABD_TO_BUF(zap->zap_dbuf->db_abd), zap->zap_dbuf->db_size);
    ...
    dmu_tx_t *tx)
{
	...
	transfer_func(ABD_TO_BUF(db_old->db_abd),
    ...
    zap_leaf_t **lp)
{
	...
	ASSERT3P(l->l_phys, ==, ABD_TO_BUF(l->l_dbuf->db_abd));
...
zap_deref_leaf(zap_t *zap, uint64_t h, dmu_tx_t *tx, krw_t lt, zap_leaf_t **lp)
{
	...
	ASSERT(zap->zap_dbuf == NULL ||
	    zap->zap_f.zap_phys == ABD_TO_BUF(zap->zap_dbuf->db_abd));
